...
string plusBitStr(string s1, string s2) //assuming those 2 strings have the same length, they have to
{
	bool carried = false;
	string sum(128, 0);
	int i;
...
string getTwoComplement(string temp)
{
	string str(temp);
	int n = str.length();
...
void shiftStrLeft_by1(string& str)
{
...
void shiftStrRight_by1(string& str)
{
...
string normalizeString(string str, int radix)
{
		if (radix == 2)
		{
			string temp;
...
QInt operator/(const QInt& q1, const QInt& q2) //assuming all strings here have the same length, they have to
{
...
void convertToNegativeBin(string& bin)
{
	...
	while (bin.length() < 128) bin = "1" + bin;
	bin = normalizeString(bin, 2);
	string one = normalizeString("1", 2);
	bin = plusBitStr(bin, one);
}

QInt::QInt(string str, int radix)
{
	_high = 0;
	_low = 0;
	if (!str.empty())
...
QInt& QInt::operator<<(const int& q)
{
...
string QInt::printQINT(int radix)
{
